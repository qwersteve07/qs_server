...
const login = async (ctx) => {
	...
	const user = socialDanceUsers.find(
		(u) => u.username === parseData.username,
	);
...
const fetchEventsData = (query) => {
	...
	const result = db.data.events.filter((event) => {
		...
		if (requestMonth === 12) {
			// 要取得後一年一月的 event
			return (
				matchCurrentMonth || matchLastMonth || matchNextYearFirstMonth
			);
		} else if (requestMonth === 1) {
			return (
				matchCurrentMonth || matchNextMonth || matchLastYearLastMonth
			);
		} else {
